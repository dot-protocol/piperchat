...
//
// v1.2 (E2E encrypted) envelope format: client sends
//   { version:"1.2", from_dot1, from_ed25519_pub, from_x25519_pub, cipher_body, wraps[], sig, ... }
//   canonical signing input: see signV12Envelope / verifyV12Envelope below.
//   The server verifies the sig but cannot decrypt cipher_body.
...
// ── v1.2 E2E-encrypted envelope helpers ──────────────────────────────────────

/**
 * Build the canonical signing input for a v1.2 envelope.
 *
 * The signed bytes cover everything EXCEPT the `sig` field itself.
 * This includes cipher_body, wraps[], from_x25519_pub — i.e. the server
 * verifies the ECDH public key and the encrypted blob are authentic.
 *
 * Attachment manifest uses the same metadata-only pattern as v1.1.
 *
 * @param {object} envelope - v1.2 envelope (without sig, or sig will be ignored)
 * @param {Array}  atts     - validated attachments array
 * @returns {Buffer}
 */
function v12CanonicalBytes(envelope, atts) {
  // Build signable fields — sorted keys, no whitespace
  const signable = {
    channel:          envelope.channel,
    cipher_body:      envelope.cipher_body,
    createdAt:        envelope.createdAt,
    from_dot1:        envelope.from_dot1,
    from_ed25519_pub: envelope.from_ed25519_pub,
    from_x25519_pub:  envelope.from_x25519_pub,
    id:               envelope.id,
    prev:             envelope.prev ?? null,
    version:          '1.2',
    wraps:            envelope.wraps,  // array of {recipient_dot1, wrapped_body_key}
  };
...
/**
 * Detect whether a request body is a v1.2 E2E-encrypted envelope.
 * @param {object} body
 */
function isV12Envelope(body) {
  return (
    typeof body === 'object' && body !== null &&
    body.version === '1.2' &&
    typeof body.from_dot1        === 'string' && /^dot1:[0-9a-f]{16}$/.test(body.from_dot1) &&
    typeof body.from_ed25519_pub === 'string' && body.from_ed25519_pub.length === 64 &&
    typeof body.from_x25519_pub  === 'string' && body.from_x25519_pub.length  === 64 &&
    typeof body.cipher_body      === 'string' && body.cipher_body.length > 0 &&
    Array.isArray(body.wraps)    && body.wraps.length >= 1 &&
    typeof body.sig              === 'string' && body.sig.length === 128
  );
}

/**
 * Sign a v1.2 envelope object. envelope must have all fields except `sig`.
 *
 * @param {object} envelope        - v1.2 envelope (without sig)
 * @param {Array}  atts            - validated attachments array
 * @param {string} ed25519PrivHex  - 128-hex ed25519 private key
 * @returns {string}               - 128-hex signature
 */
function signV12Envelope(envelope, atts, ed25519PrivHex) {
  const privBytes = Buffer.from(ed25519PrivHex, 'hex');
  const bytes     = v12CanonicalBytes(envelope, atts);
  const sig       = nacl.sign.detached(bytes, privBytes);
  return Buffer.from(sig).toString('hex');
}
 ...
 *
 * @param {object} envelope - v1.2 envelope (must include all fields including sig)
 * @param {Array}  atts     - validated attachments array
 * @returns {{ ok: true } | { ok: false, reason: string }}
 */
function verifyV12Envelope(envelope, atts) {
  if (!isV12Envelope(envelope)) {
    return { ok: false, reason: 'missing or malformed v1.2 envelope fields' };
  }

  const { from_ed25519_pub, sig } = envelope;
  ...
  if (pubkeyBytes.length !== 32) return { ok: false, reason: 'from_ed25519_pub must be 32 bytes (64 hex)' };
  if (sigBytes.length    !== 64) return { ok: false, reason: 'sig must be 64 bytes (128 hex)' };

  const msgBytes = v12CanonicalBytes(envelope, atts);
  const valid    = nacl.sign.detached.verify(msgBytes, sigBytes, pubkeyBytes);

  if (!valid) return { ok: false, reason: 'v1.2 signature verification failed' };
  return { ok: true };
}
